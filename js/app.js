...
const updateDataKeyboard = (dataKB = {}, action) => {
  switch (action) {
    case ACTION_LIST.SET_LANGUAGE:
      const languageStatus = changeLanguage(dataKeyboard);
...
const keybord = {
  ...
  checkActiveClass(name) {
    ...
    switch (name) {
      case 'capslock':
        if (this.capslockPress) res = '_active';
        break;

      case 'shiftleft':
        if (this.shiftPres) res = 'active';
        break;

      case 'shiftright':
        if (this.rightShiftPres) res = 'active';
        break;
  ...
  reCreateKeys() {
    ...
    this.keysLayout.forEach((key, i) => {
      ...
      if (['capslock', 'shiftright', 'shiftleft'].indexOf(key) !== -1) {
        const cls = this.checkActiveClass(key);
        if (cls) keyElem.classList.add(cls);
      }
      ...
      switch (key) {
        ...
        case 'capslock':
          ...
          keyElem.addEventListener('click', () => {
            this.capslockPress = !this.capslockPress;
            // this.toogleCapsLock();
            keyElem.classList.add('active');
        ...
        case 'shiftright':
          ...
          keyElem.addEventListener('click', () => {
            this.rightShiftPres = !this.rightShiftPres;
            updateDataKeyboard(dataKeyboard, ACTION_LIST.SET_SHIFT_STATUS);
            this.keysLayout = dataKeyboard.activeKeyBoard;
            keybord.reload();
            keyElem.classList.toggle('active');
        ...
        default:
          ...
          window.addEventListener('keydown', (e) => {
            const ind = dataKeyboard.allLetters.indexOf(e.key);
